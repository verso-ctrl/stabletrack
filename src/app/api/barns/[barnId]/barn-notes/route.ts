...
export async function GET(req: NextRequest, context: RouteContext) {
  try {
    ...
    const notes = await prisma.barnNote.findMany({
      where: { barnId },
    ...
    console.error('Error fetching barn notes:', error)
...
export async function POST(req: NextRequest, context: RouteContext) {
  try {
    ...
    const note = await prisma.barnNote.create({
      data: { barnId, content, createdBy: user.id },
      include: { author: { select: { firstName: true, lastName: true } } },
    })
    ...
    console.error('Error creating barn note:', error)
